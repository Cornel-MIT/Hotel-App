...
const AddRoom = () => {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '500px', margin: '0 auto' }}>
      ...
      <button onClick={() => navigate('/admin')} style={{ marginTop: '10px', padding: '10px', backgroundColor: '#f44336', color: 'white', border: 'none', cursor: 'pointer' }}>Back</button>
      {showPopup && (
        <div style={popupStyle}>
          <p>Congratulations! You Successfully Added a New Room.</p>
          <button onClick={() => setShowPopup(false)} style={{ padding: '5px', backgroundColor: '#008CBA', color: 'white', border: 'none', cursor: 'pointer' }}>Close</button>
        </div>
      )}
    </div>
  );
